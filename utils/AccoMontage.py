...
from models.ptvae import RnnEncoder,TextureEncoder,RnnDecoder,PtvaeDecoderwithAtt,PtvaeDecoder
...
def render_acc(pianoRoll, chord_table, query_seg, indices, shifts, acc_pool, model_path):
...
#    pt_decoder = PtvaeDecoderwithAtt(note_embedding=None, dec_dur_hid_size=64, z_size=512)
    ...
    midiReGen = accomapnimentGeneration(pianoRoll, est_x, 120, with_arousal_evaluation=False)
    return midiReGen
...
def get_texture_filter(acc_pool):
    ...
    for key in acc_pool:
        ...
        onset_positions = (np.sum(acc_track, axis=-1) > 0)
        HD = np.sum(onset_positions, axis=-1) / acc_track.shape[1]
        simu_notes = np.sum((acc_track > 0) , axis=-1)
        VD = np.sum(simu_notes, axis=-1) / (np.sum(onset_positions, axis=-1) + 1e-10)
        dst = np.sort(HD)
        HD_anchors = [dst[len(dst)//10*i] for i in range(1,10)]
        HD_Bins = [
        HD < HD_anchors[0], 
        (HD >= HD_anchors[0]) * (HD < HD_anchors[1]), 
        (HD >= HD_anchors[1]) * (HD < HD_anchors[2]), 
        (HD >= HD_anchors[2]) * (HD < HD_anchors[3]),
            (HD >= HD_anchors[3]) * (HD < HD_anchors[4]),
            (HD >= HD_anchors[5]) * (HD < HD_anchors[6]),
            (HD >= HD_anchors[6]) * (HD < HD_anchors[7]),
            (HD >= HD_anchors[7]) * (HD < HD_anchors[8]),
            HD >= HD_anchors[8]
        ]

        dst = np.sort(VD)
        VD_anchors = [dst[len(dst)//10*i] for i in range(1,10)]
        VD_Bins = [
        VD < VD_anchors[0], 
        (VD >= VD_anchors[0]) * (VD < VD_anchors[1]), 
        (VD >= VD_anchors[1]) * (VD < VD_anchors[2]), 
        (VD >= VD_anchors[2]) * (VD < VD_anchors[3]),
        (VD >= VD_anchors[3]) * (VD < VD_anchors[4]),
        (VD >= VD_anchors[4]) * (VD < VD_anchors[5]),
        (VD >= VD_anchors[5]) * (VD < VD_anchors[6]),
        (VD >= VD_anchors[6]) * (VD < VD_anchors[7]),
        (VD >= VD_anchors[7]) * (VD < VD_anchors[8]),
        VD >= VD_anchors[8]
        ]
