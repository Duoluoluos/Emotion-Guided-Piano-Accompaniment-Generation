...
import numpy as np
from models.ptvae import RnnEncoder, RnnDecoder, PtvaeDecoderwithAtt, \
    TextureEncoder
from config import *
...
class DisentangleVAE(PytorchModel):
    ...
    def run(self, x, c, pr_mat, tfr1, tfr2, tfr3, confuse=True):
        ...
        pitch_outs, dur_outs = self.decoder(dec_z, False, embedded_x,
                                            lengths, tfr1, tfr2)
        recon_root, recon_valence, recon_bass = self.chd_decoder(z_chd, False,
                                                                tfr3, c)
        return pitch_outs, dur_outs, dist_chd, dist_rhy, recon_root, \
            recon_valence, recon_bass

    def loss_function(self, x, c, recon_pitch, recon_dur, dist_chd,
                      dist_rhy, recon_root, recon_valence, recon_bass,
                      beta, weights, weighted_dur=False):
        arousal_loss, pl, dl = self.decoder.recon_loss(x, recon_pitch, recon_dur,
                                                     weights, weighted_dur)
        kl_loss, kl_chd, kl_rhy = self.kl_loss(dist_chd, dist_rhy)
        chord_loss, root, chroma, bass = self.chord_loss(c, recon_root.to(self.device),
                                                         recon_valence.to(self.device),
                                                         recon_bass.to(self.device))
        loss = arousal_loss + beta * kl_loss + chord_loss
        return loss, arousal_loss, pl, dl, kl_loss, kl_chd, kl_rhy, chord_loss, \
               root, chroma, bass

    def chord_loss(self, c, recon_root, recon_valence, recon_bass):
        ...
        recon_valence = recon_valence.view(-1, 2).contiguous()
        recon_bass = recon_bass.view(-1, 12).contiguous()
        root_loss = loss_fun(recon_root, root)
        valence_loss = loss_fun(recon_valence, chroma)
        bass_loss = loss_fun(recon_bass, bass)
        chord_loss = root_loss + valence_loss + bass_loss
        return chord_loss, root_loss, valence_loss, bass_loss
...
class BiLSTM(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=12,hidden_size=RNN_SIZE,
                            num_layers=NUM_LAYERS,batch_first=True,
                            dropout=0.2,bidirectional=True)
        self.output = nn.Linear(256,25)
    def forward(self,x):
        x = self.fc1(x)
        x, _ = self.lstm(x)
