...
class PtvaeDecoderwithAtt(nn.Module):
    ...
    def __init__(self, device=None, note_embedding=None,
                 ...
                 dec_z_in_size=256, dec_dur_hid_size=16,attention_emb=32,kvdim=128):
        super(PtvaeDecoderwithAtt, self).__init__()
        # Parameters
        # note and time
        self.max_pitch = max_pitch  # the highest pitch in train/val set.
        self.min_pitch = min_pitch  # the lowest pitch in train/val set.
        ...
        self.dec_time_gru = \
            nn.GRU(dec_z_in_size + 2 * dec_emb_hid_size,
                   ...
                   num_layers=1, batch_first=True,
                   bidirectional=False)
        self.note_summary_attention = NoteSummaryAttention(dec_time_hid_size, kvdim,
    ...
    def pitch_dur_ind_to_note_token(self, pitch_inds, dur_inds, batch_size):
        token = torch.zeros(batch_size, self.note_size).to(self.device)
    ...
    def decoder(self, z, inference, x, lengths, teacher_forcing_ratio1,
        ...
        for t in range(self.num_step):
            ...
            notes_summary = self.note_summary_attention(torch.squeeze(notes_summary, dim=1))
            if inference:
    ...
    def recon_loss(self, x, recon_pitch, recon_dur, weights=(1, 0.5),
        ...
        recon_pitch = recon_pitch.view(-1, recon_pitch.size(-1)).to(self.device)
...
class NoteSummaryAttention(nn.Module):

    def __init__(self, q_dim=1024, kv_dim=128, emb_dim=32, output_dim=1024, num_heads=1):
        super(NoteSummaryAttention, self).__init__()
        self.q_dim = q_dim
        self.kv_dim = kv_dim
        self.num_heads = num_heads
        self.emb_dim = emb_dim
        self.wq = nn.Linear(q_dim, emb_dim)
        self.wk = nn.Linear(q_dim, emb_dim)
        self.wv = nn.Linear(q_dim, emb_dim)
        self.multi_attention = nn.MultiheadAttention(1, num_heads, batch_first=True)
        self.output_layer = nn.Linear(emb_dim, output_dim)

    def forward(self, x):
        q = torch.unsqueeze(self.wq(x), dim=-1)
        k = torch.unsqueeze(self.wk(x), dim=-1)
        v = torch.unsqueeze(self.wv(x), dim=-1)
        attn_output, attn_output_weights = self.multi_attention(q, k, v)
        attn_output = torch.unsqueeze(torch.squeeze(attn_output, dim=-1), dim=1)
